...
bert = BertModel.from_pretrained(r"E:\NLPWork\NLP\NLPDemo\Exercise\com\demo\week6\bert-base-chinese", return_dict=False)
state_dict = bert.state_dict()
bert.eval()
x = np.array([2450, 15486, 102, 2110, 1392])   #假想成4个字的句子
torch_x = torch.from_numpy(x)             #pytorch形式输入
torch_x = torch.unsqueeze(torch_x, 0)  #增加一维
...
class DiyBert:
    ...
    def count_parameters(self):
        total = 0
        total += self.word_embeddings.size
        total += self.position_embeddings.size
        total += self.token_type_embeddings.size
        total += self.embeddings_layer_norm_weight.size
        total += self.embeddings_layer_norm_bias.size
        # 计算 Transformer 层的参数数量
        for linear in self.transformer_weights:
            for weight in linear:
                if isinstance(weight, np.ndarray):
                    total += weight.size
        # 计算 Pooler 层的参数数量
        total += self.pooler_dense_weight.size
        total += self.pooler_dense_bias.size
        return total
...
print("可训练参数数量：", db.count_parameters())
