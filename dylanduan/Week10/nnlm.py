...
def create_mask(batch_size, num_heads, seq_len):
    ...
    # 扩展掩码以匹配期望的形状 [batch_size, num_heads, seq_len, seq_len]
    # mask = mask.unsqueeze(0).unsqueeze(0)
    # mask = mask.expand(batch_size, num_heads, seq_len, seq_len)
    
    return mask

def create_mask_1D(batch_size, num_heads, seq_len):
...
class LanguageModel(nn.Module):
    def __init__(self, input_dim, vocab):
        ...
        self.bert = BertModel.from_pretrained("/Users/duansuyang/DeepLearning/LLM_Project/bert-base-chinese", return_dict=False)
    ...
    def forward(self, x, y=None):
        ...
        if y != None:
            mask = torch.tril(torch.ones((x.shape[0], x.shape[1], x.shape[1]), device=x.device))
            x, _ = self.bert(x, mask)
        else:
            x, _ = self.bert(x)
