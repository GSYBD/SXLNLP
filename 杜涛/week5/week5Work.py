...
def main():
    model = load_word2vec_model(r"C:\Users\tao\PycharmProjects\nlp\杜涛\week5\model.w2v")  #加载词向量模型
    ...
    cluster_distance = defaultdict(list)        #保存簇间距离

    for sentence, label, vector in zip(sentences, kmeans.labels_, vectors):  #取出句子和标签
        sentence_label_dict[label].append(sentence)         #同标签的放到一起
        distance = np.linalg.norm(vector - center[label])   #计算与质心的距离
        cluster_distance[label].append(distance)  # 存储距离
