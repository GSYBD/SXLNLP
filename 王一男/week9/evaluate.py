...
class Evaluator:
    ...
    def eval(self, epoch):
        ...
        self.stats_dict = dict(zip(self.schema.keys(),  # keys,dict_int => dict{key-dict}
                                   [defaultdict(int) for i in range(len(self.schema))]))
        self.model.eval()
        for index, batch_data in enumerate(self.valid_data):   # sentence_without_sign ↓
            sentences = self.valid_data.dataset.sentences[index * self.config["batch_size"]: (index+1) * self.config["batch_size"]]
            if torch.cuda.is_available():
                batch_data = [d.cuda() for d in batch_data]
            input_id, mask, labels = batch_data   # 输入变化时这里需要修改，比如多输入，多输出的情况
            with torch.no_grad():
                pred_results = self.model(input_id, mask)  # 不输入labels，使用模型当前参数进行预测
    ...
    def write_stats(self, labels, pred_results, sentences):
        ...
        for true_label, pred_label, sentence in zip(labels, pred_results, sentences):
            ...
            for pred, correct in zip(pred_label, true_label):
                key = self.index_to_label[correct]
                self.stats_dict[key]["correct"] += 1 if pred == correct else 0
