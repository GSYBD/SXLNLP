...
class LanguageModel(nn.Module):
    def __init__(self):
        ...
        self.bert = BertModel.from_pretrained(r"E:\111绝密资料，禁止外传(2)\AIML_llm\第六周 预训练模型\bert-base-chinese", return_dict=False)
        self.config = self.bert.config
        self.classify = nn.Linear(self.config.hidden_size, self.config.vocab_size)
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            # 构建一个下三角的mask
            # bert的mask attention 为(batch_size, seq_len, seq_len) L*L
...
def train(corpus_path, save_weight=True):
    ...
    tokenizer = BertTokenizer.from_pretrained(r"E:\111绝密资料，禁止外传(2)\AIML_llm\第六周 预训练模型\bert-base-chinese")
    if torch.cuda.is_available():
        model = model.cuda()
...
if __name__ == "__main__":
    train(r"E:\111绝密资料，禁止外传(2)\AIML_llm\第十周 生成式任务\lstm语言模型生成文本\corpus.txt", False)
