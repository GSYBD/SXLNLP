...
规律2：x是一个5维向量，向量中哪个标量更接近于1，就输出哪一维下标
...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        #self.activation = torch.tanh  # tanh函数
...
# 随机生成一个5维向量，找出每个标量和1的差值最小的下标，返回X,Y
def build_sample():
    x = np.random.random(5)
    #target_value = x[0]
    differences = np.abs(x - 1)
...
def main():
    ...
    # 保存模型
    torch.save(model.state_dict(), "model_for_cross_entropy.pt")
    # 画图
    print(log)
...
if __name__ == "__main__":
    ...
    predict("model_for_cross_entropy.pt", test_vec)
