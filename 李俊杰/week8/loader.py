...
class DataGenerator:
    ...
    def __getitem__(self, index):
        if self.data_type == "train":
            return self.random_train_three_sample()  # 随机生成一个训练样本
    ...
    # TODO:work
    def random_train_three_sample(self):
        standard_question_index = list(self.knwb.keys())
        lable1, lable2 = random.sample(standard_question_index, 2)
        while len(self.knwb[lable1]) < 2 and lable1 != lable2:
            lable1 = random.choice(standard_question_index)
        a, p = random.sample(self.knwb[lable1], 2)
        n = random.choice(self.knwb[lable2])
        return [a, p, n]
