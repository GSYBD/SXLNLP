...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, sentence_length, vocab):
        ...
        self.classify = nn.Linear(sentence_length, sentence_length + 1)  # 线性层
        # self.activation = torch.sigmoid  # sigmoid归一化函数
        self.loss = nn.CrossEntropyLoss()  # loss函数采用均方差损失

    # 当输入真实标签，返回loss值；无真实标签，返回预测值
    def forward(self, x, y=None):
        x = self.embedding(x)  # (batch_size, sen_len) -> (batch_size, sen_len, vector_dim) 2*6*20
        # x = self.BN(6)
        # x = x.transpose(1, 2)  # (batch_size, sen_len, vector_dim) -> (batch_size, vector_dim, sen_len)
        # x = self.pool(x)  # (batch_size, vector_dim, sen_len)->(batch_size, vector_dim, 1)
        x = self.layer(x)  # RNN输出元祖
        x = x[1]  # 1为池化
        x = x.squeeze()
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        ...
        print("输入：%s, 预测类别：%d" % (input_string, max_index))  # 打印结果


if __name__ == "__main__":
    # main()
    test_strings = ["anvfee", "wasdfg", "rqadeg", "nakwww", "nsssss"]
