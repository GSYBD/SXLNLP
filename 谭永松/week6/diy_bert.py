...
config_file_path = r"D:\tys\note\1.ai\资料库\第二周\week2 深度学习基本原理\week6 语言模型和预训练\bert-base-chinese\config.json"
...
# 嵌入层
EmbeddingTotal = vocab_size * hidden_size + max_position_embeddings * hidden_size + type_vocab_size * hidden_size
# 编码器层参数总数
EncoderLayer = (3 * (hidden_size * hidden_size + hidden_size) + (hidden_size * hidden_size + hidden_size)
                + (hidden_size * intermediate_size + intermediate_size) + (intermediate_size * hidden_size))
EncoderTotal = num_hidden_layers * EncoderLayer
# 分类器参数总数
Classifier = hidden_size * num_labels + num_labels
total_parameters = EmbeddingTotal + EncoderTotal + Classifier
