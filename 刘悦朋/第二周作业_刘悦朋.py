...
def build_sample(dim=5):
    ...
    # label = np.array([0] * (list(x).index(max(x))) + [1] + [0] * (dim - list(x).index(max(x)) - 1))
    label = list(x).index(max(x))
    return x, label
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if list(y_p).index(max(y_p)) == int(y_t):
                correct += 1
            else:
                wrong += 1
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        print("输入：%s, 预测类别：%d, 概率值：%f" % (vec, list(res).index(max(res)), res[list(res).index(max(res))]))  # 打印结果
