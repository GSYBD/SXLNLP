...
class TorchModel(nn.Module):
    ...
    def forward(self, x, target=None):
        if self.use_bert:  # bert返回的结果是 (sequence_output, pooler_output)
            ...
            #pooler_output:batch_size, hidden_size。每句话对应的向量
            x = self.encoder(x)#bert自带embedding
        else:
...
class BertLSTM(nn.Module):
    ...
    def forward(self, x):
        x = self.bert(x)[0]#bert输出词向量和句向量，这里取词向量
        x, _ = self.rnn(x)
        return x
