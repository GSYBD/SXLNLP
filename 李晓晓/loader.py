    ...
    def random_train_sample(self):
        ...
        if random.random() <= self.config["positive_sample_rate"]:
            ...
            if len(self.knwb[p]) < 2:
                ...
                a_s, p_s = random.sample(self.knwb[p], 2)
                return [a_s, p_s, torch.LongTensor([1])]
        #随机负样本
        if random.random() > self.config["positive_sample_rate"]:
            n = random.sample(standard_question_index)
            if n != p:
                n_s = random.choice(self.knwb[p])
                return [n_s, torch.LongTensor([-1])]
