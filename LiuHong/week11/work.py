...
def build_sample(tokenizer, corpus, max_length=150):
    ...
    input_seq = tokenizer.encode(title + "[SEP]" + content, add_special_tokens=False, padding='max_length',
                                 truncation=True, max_length=max_length)  # 输入序列
    output_seq = tokenizer.encode(("[PAD]" * len(title)) + content + "[CLS]", add_special_tokens=False,
                                  padding='max_length', truncation=True, max_length=max_length)  # 输出序列
...
def train(corpus_path, save_weight=True):
    ...
    pretrain_model_path = r"/Users/lh/PycharmProjects/nlp/week6/bert-base-chinese"
