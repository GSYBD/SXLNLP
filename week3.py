...
def build_sample(vocab, sentence_length):
    ...
    x = x[:random_index] + ["你"] + x[random_index + 1:]
    y = x.index("你")
...
def build_model(input_size, hidden_size):
    ...
    model = DiyModel(w_ih, w_hh, hidden_size)
    return model
...
def evaluate(model, vocab, sample_length):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if int(y_t)+1 > torch.argmax(y_p) > int(y_t)-1:
                correct += 1  # 负样本判断正确
            else:
                wrong += 1
...
if __name__ == "__main__":
    main()
    test_strings = ["fn你fee", "w你sdfg", "rqw你eg", "nakww你"]
