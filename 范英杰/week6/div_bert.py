...
class DiyBert:
    ...
    def get_layer_argument(self, input_size,intermediate_weight_size) :
        '''
        embeddings_argument: input_size * hidden_size + max_position_embeddings*hidden_size + type_vocab_size*hidden_size
        '''
        # word_embeddings = self.word_embeddings # we input_size * hidden_size
        # position_embeddings = self.position_embeddings # pe 512*hidden
        # token_type_embeddings = self.token_type_embeddings # te 2*hidden
        '''
        self_attention_argument: num_layers * (hidden_size*hidden_size*3 + hidden_size*3)
        '''
        # q_w = self.q_w
        # q_b = self.q_b
        # k_w = self.k_w
        ...
        # v_b = self.v_b
        '''
        bn层:hidden_size*hidden_size + hidden_size
        '''
        # attention_layer_norm_w = self.attention_layer_norm_w
        # attention_layer_norm_b = self.attention_layer_norm_b
        '''
        feed forward层 (intermediate_size*hidden_size + intermediate_size)*2
        '''
        # intermediate_weight = self.intermediate_weight
        '''
        bn层:hidden_size*hidden_size + hidden_size
        '''
        # ff_layer_norm_w = self.ff_layer_norm_w
        # ff_layer_norm_b = self.ff_layer_norm_b
        '''
        #链接[cls] token的输出层 hidden_size*hidden_size + hidden_size
        '''
        # pooler_dense_weight = self.pooler_dense_weight
        # pooler_dense_bias = self.pooler_dense_bias

        '''
        argument_size =vocab_size * hidden_size + max_position_embeddings*hidden_size + type_vocab_size*hidden_size  + num_layers * (hidden_size*hidden_size*3 + hidden_size*3)
        +(hidden_size*hidden_size + hidden_size)*3 + (intermediate_size*hidden_size + intermediate_size)*2
        '''

        return 3*(self.num_layers + 1)*self.hidden_size*self.hidden_size +(input_size + self.position_embeddings.size + self.token_type_embeddings.size  + 2*intermediate_weight_size + 6)*self.hidden_size + 2*intermediate_weight_size

#vocab_size * hidden_size + max_position_embeddings*hidden_size + type_vocab_size*hidden_size  + num_layers * (hidden_size*hidden_size*3 + hidden_size*3)
# (hidden_size*hidden_size + hidden_size)*3 + (intermediate_size*hidden_size + intermediate_size)*2
...
size = db.get_layer_argument(x.size,3072)
print(size)
