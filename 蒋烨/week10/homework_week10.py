...
class LanguageModel(nn.Module):
    def __init__(self, hidde_size, vocab_size, bert_path):
        super(LanguageModel, self).__init__()
        self.bert = BertModel.from_pretrained(bert_path, return_dict=False)
        self.classify = nn.Linear(hidde_size, vocab_size)
...
#建立模型
def build_model(vocab_size, char_dim, bert_path):
    model = LanguageModel(vocab_size, char_dim, bert_path)
    return model
...
def train(corpus_path, save_weight=True):
    ...
    vocab_size = 21128       #建立字表
    corpus = load_corpus(corpus_path)     #加载语料

    bert_path = "bert-base-chinese"
    tokenizer = BertTokenizer.from_pretrained(bert_path)

    model = build_model(vocab_size, char_dim, bert_path)    #建立模型
    if torch.cuda.is_available():
        model = model.cuda()
