...
class DataGenerator:
    ...
    def __getitem__(self, index):
        if self.data_type == "train":
            if self.config["loss_flag"] == "triplet":
                return self.random_train_sample_4triplet() #随机生成一个训练样本
            else:
    ...
    # choose sample for triplet 
    # 样本构成：
    #   随机选取一个问题a
    #   从随机一个标准问题中随机选取一个该问题a正样本p
    #   从随机一个标准问题中随机选取一个负样本n
    def random_train_sample_4triplet(self):
        standard_question_index = list(self.knwb.keys())
        # 生成随机样本a,p,n
        # 随机正样本a-s1,p-s2
        p = random.choice(standard_question_index)
        #如果选取到的标准问下不足两个问题，则无法选取，所以重新随机一次
        if len(self.knwb[p]) < 2:
            return self.random_train_sample_4triplet()
        else:
            s1, s2 = random.sample(self.knwb[p], 2)
        #随机负样本n-s3
