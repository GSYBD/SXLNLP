...
class SiameseNetwork(nn.Module):
    def __init__(self, config):
        ...
        # 定义调用不同的loss函数
        if config["loss_flag"] == "cosine":
            self.loss = nn.CosineEmbeddingLoss()
        elif config["loss_flag"] == "triplet":
            self.loss = nn.TripletMarginLoss(margin=0.1)
        else:
            raise ValueError("loss_flag 只能是 cosine 或者 triplet")
    ...
    输入
        使用三元组损失时，
            anchor sentence1: 问题1编码
            positive sentence2: 问题2编码
            negative sentence3: 问题3编码
    输出：
        loss: 损失
    ...
    def forward(self, sentence1, sentence2=None, sentence3=None, target=None):
        ...
        if sentence2 is not None:
            ...
            #如果有标签，则计算loss
                ...
                #计算三元组损失
            if self.config["loss_flag"] == "triplet" and sentence3 is not None:
                vector3 = self.sentence_encoder(sentence3)
                loss = self.loss(vector1, vector2,vector3)
                return loss
            if target is not None:
                #计算余弦损失
                if self.config["loss_flag"] == "cosine":
                    loss = self.loss(vector1, vector2, target.squeeze())
                    return loss
                else:
                    raise ValueError("loss_flag 只能是 cosine 或者 triplet")
            #如果无标签，计算余弦距离
