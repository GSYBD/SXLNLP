...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.layer1 = nn.Linear(input_size, 5)  # 线性层
    ...
    def forward(self, x, y=None):
        ...
        y_pred = self.layer2(x)  # (batch_size, 1) -> (batch_size, 1)
...
def build_sample():
    ...
    y = [0, 0, 0, 0, 0]
    min_value = x.min()
    min_index = np.unravel_index(x.argmin(), x.shape)
    # print(f"随机生成的5维向量 最大值: {max_value}, 最大值下标: {max_index}")
    y[min_index[0]] = 1
    return x, y
...
def main():
    ...
    # 保存模型
    torch.save(model.state_dict(), "week2m.pt")
    # 画图
    print(log)
