...
class Evaluator:
    ...
    def knwb_to_vector(self):
        ...
        self.question_ids = []
        # print(self.train_data.dataset.knwb)
        # exit()
        for standard_question_index, question_ids in self.train_data.dataset.knwb.items():
            for question_id in question_ids:
                """
                就是一个记录所有的输入id，另一个记录每个索引所对应的类别
                
                1878*128
                tensor([[0.0190, 0.0754, 0.0546,  ..., 0.0390, 0.0691, 0.0498],
        [0.1002, 0.1218, 0.0427,  ..., 0.0815, 0.0385, 0.1708],
        [0.0879, 0.1068, 0.0374,  ..., 0.0714, 0.0626, 0.1497],
        ...,
        [0.0615, 0.0779, 0.0874,  ..., 0.0421, 0.0556, 0.0200],
        [0.0311, 0.0576, 0.0789,  ..., 0.0651, 0.1020, 0.0528],
        [0.0323, 0.0755, 0.0818,  ..., 0.0911, 0.0521, 0.0455]])
                """
    ...
    def write_stats(self, test_question_vectors, labels):
        ...
        # exit()
        """
        test_question_vectors:32 * 128
        """
        for test_question_vector, label in zip(test_question_vectors, labels):
            # 通过一次矩阵乘法，计算输入问题和知识库中所有问题的相似度
            # 1 * 128  mm 128 * 1878  得到这句话关于每个句子的相似度
            # test_question_vector shape [vec_size]   knwb_vectors shape = [n, vec_size]
            # print(test_question_vector.shape,test_question_vector.unsqueeze(0))
            # print(self.knwb_vectors.T.shape,test_question_vector.unsqueeze(0).shape)
            # exit()
