...
class TorchModel(nn.Module):
    ...
    self.encoder = lambda x: x 输入什么就输出什么
    lstm，gru，rnn  输出维度 hidden_size
    CNN ： 输出 258行？
    RCNN：先过rnn，再过cnn层

    bert_lstm bertxx ：就是bert之后，使用词向量过接下来的层
    只不过隐藏层的维度就是使用bert的，出来的是 l * h 取用所有的，不是池化的
    bert_mid_layer：去隐藏层最后两层的结果相加
    """
    ...
    def forward(self, x, target=None):
        # print(x,x.shape)
        """
        bert : shuchu quanbu 128, 30, 768 
        pool 128, 768
        最后只要输出层
        
         nn.MaxPool1d(x.shape[1]) 设置池化窗口的大小
         pool是池化最后一行，所以先转置，池化完了再转回来
         最终30 * hidden 池化成 1 * hidden
...
class StackGatedCNN(nn.Module):
    ...
    有点类似bert的transformert，多个做残差，过归一化层
...
class BertMidLayer(nn.Module):
    def __init__(self, config):
        ...
        self.bert = BertModel.from_pretrained(config["pretrain_model_path"], return_dict=False)
        # output_hidden_states 输出中间隐藏层，就是self.bert(x)[2]，所有隐藏层
