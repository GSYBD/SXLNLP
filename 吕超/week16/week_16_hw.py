...
"""
week_16_hw.py
Created on 2024/10/23 14:18
@author: Allan Lyu
@version: v1.0
@Description: week_16_hw 为对话系统加入重听能力
"""
...
class DialogSystem:
    def __init__(self):
        self.load()
        self.last_bot_response = None  # 用于存储上一次的机器人回复
    ...
    def generate_response(self, user_input, memory):
        ...
        self.last_bot_response = memory["bot_response"]  # 更新上一次的机器人回复
        return memory["bot_response"], memory

    def is_repeat_request(self, user_input):
        # 定义重听请求的关键字
        repeat_keywords = ["你说什么？", "我没听清", "重复一下", "再说一遍"]
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if ds.is_repeat_request(user_input):
            # 如果用户请求重听，则重复上一次的回复
            response = ds.last_bot_response
        else:
            # 否则，生成新的回复
