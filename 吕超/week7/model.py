...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        if model_type == "fast_text":   # lyu:fast_text怎么理解???
            self.encoder = lambda x: x
        elif model_type == "lstm":  # lyu:TextRNN使用RNN（LSTM、GRU）对文本进行编码，使用最后一个位置的输出向量进行分类
            ...
            self.use_bert = True
            self.encoder = BertMidLayer(config)  # lyu:  具体含义是啥?
            hidden_size = self.encoder.bert.config.hidden_size

        self.classify = nn.Linear(hidden_size, class_num) #lyu: 全连接层, 输出分类结果, 比如768映射到10类
    ...
    def forward(self, x, target=None):
        if self.use_bert:  # bert返回的结果是 (sequence_output, pooler_output)
            ...
            x = self.encoder(x)  # lyu: bert自带embdding, 如果模型使用bert, 参数直接传入模型即可, 其他模型需要先传入embdding层
        else:
            x = self.embedding(x)  # input shape:(batch_size, sen_len)
            x = self.encoder(x)  # input shape:(batch_size, sen_len, input_dim)

        # lyu: 适配两种输出方式,即bert/LSTM的输出是tuple, 其他模型如CNN输出是tensor, 默认取出第一个元素
        if isinstance(x, tuple):  #RNN类的模型会同时返回隐单元向量，我们只取序列结果
            x = x[0]
        #可以采用pooling的方式得到句向量  # lyu: 两种pooling方式, maxpooling和avgpooling
        ...
        #也可以直接使用序列最后一个位置的向量 # lyu: LSTM使用最后一个位置的向量, bert不需要这么处理
        # x = x[:, -1, :]
...
class BertLSTM(nn.Module):
    ...
    def forward(self, x):
        x = self.bert(x)[0] # lyu: 取出bert的第一个元素, 送入RNN
        x, _ = self.rnn(x)
        return x
