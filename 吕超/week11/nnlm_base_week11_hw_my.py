...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, mask=None, y=None):
        if y is not None:
            ...
            # print('11111111111111111111',y.shape)
            # print('4444444444444444444444',y.view(-1).shape)
            # print('2222222222222222222222222',y_pred.shape)
            # print('333333333333333333333333333',y_pred.shape[-1])
...
#
#     # lyu: 修改mask形状, mask形状不固定, 实现seq2seq形式, 训练问答对,问题之间可以看到, 答案前面一个字看不到后面的字, 由4部分进行拼接
#
#     return x, y
...
def train(corpus_path, save_weight=True):
    epoch_num = 20  # 训练轮数
    batch_size = 128  # 每次训练样本个数  # lyu: 区别 128 或 2
    ...
    for epoch in range(epoch_num):
        ...
        for x, mask, y in train_data:  # 构建一组训练样本
            # train_data = build_dataset(tokenizer, corpus, window_size, batch_size)  # 构建一组训练样本
            if torch.cuda.is_available():
