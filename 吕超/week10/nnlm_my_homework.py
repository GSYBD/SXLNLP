...
class LanguageModel(nn.Module):
    def __init__(self, input_dim, vocab_size, pretrain_model_path):
        ...
        self.classify = nn.Linear(input_dim, vocab_size) # lyu: 映射到词表大小, 在词表上做的分类
    ...
    #     shape = mask.shape
    #     if len(shape) == 1:
    #         key_value_length = shape[0]
    #     elif len(shape) == 2:
    #         _, key_value_length = shape
    #     else:
...
#     window = corpus[start:end] # lyu: 截取窗口,从随机的开始 到结束
#     target = corpus[start + 1:end + 1]  #输入输出错开一位 lyu: 输入是前n个字，输出是第n+1个字
#     # print(window, target)
...
#     # reverse_vocab = dict((y, x) for x, y in vocab.items()) # lyu: 需要根据序号得到字
#     model.eval()
#     with torch.no_grad():
#         pred_char = ""
...
#             x = torch.LongTensor([x]) # lyu: 1条样本, batch_size=1
#             if torch.cuda.is_available():
#                 x = x.cuda()
#             y = model(x)[0][-1] # lyu: 0 表示取出第1个样本, -1表示取出最后一个字对应的向量
...
def sampling_strategy(prob_distribution):
    if random.random() > 0.1: # lyu: 90%的概率使用贪婪方式
        strategy = "sampling"
        strategy = "greedy"
    else:
        strategy = "sampling"
    if strategy == "greedy":
        return int(torch.argmax(prob_distribution)) # lyu: 贪婪方式取概率最大的序号
    elif strategy == "sampling":
        prob_distribution = prob_distribution.cpu().numpy() # lyu: 根据概率分布进行采样
