...
class LanguageModel(nn.Module):
    def __init__(self):
        ...
        self.encoder = BertModel.from_pretrained(r"D:\资料\week9 序列标注问题\week9 序列标注问题\ner\bert-base-chinese",
                                                 return_dict=False)
        self.hidden_size = self.encoder.config.hidden_size
        self.classify = nn.Linear(self.hidden_size, self.encoder.config.vocab_size)
...
def generate_sentence(openings, model, window_size, tokenizer):
    ...
    with torch.no_grad():
        ...
        while pred_char != "\n" and len(openings) <= 30:
            openings += pred_char
            x = [tokenizer.encode(char, add_special_tokens=False, padding='max_length', truncation=True,
                                  max_length=window_size) for char in openings[-window_size:]]
...
def train(corpus_path, save_weight=False):
    ...
    tokenizer = BertTokenizer.from_pretrained(r"D:\资料\week9 序列标注问题\week9 序列标注问题\ner\bert-base-chinese")
    if torch.cuda.is_available():
        model = model.cuda()
