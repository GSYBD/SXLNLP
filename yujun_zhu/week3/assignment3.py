...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        y_pred = self.classify(x)                  #(batch_size, vector_dim) -> (batch_size, 5)
        if y is not None:
...
def evaluate(model, vocab, sample_length):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  #与真实标签进行对比
            if torch.argmax(y_p) == y_t:  #argmax取出最大值的位置
                correct += 1
            else:
                wrong += 1
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        print("输入：%s, 预测类别：%d, 概率值：%f" % (input_string, torch.argmax(result[i]).item(), torch.max(torch.softmax(result[i], 0)).item())) #打印结果
