...
规律：x是一个5维向量，哪个最大就是输出哪个的下标01234

使用的优化器为 adam
使用的loss函数不用sgd 而是使用 crossEntropy

激活函数-Softmax  multi-class classification
...
# 随机生成一个5维向量，如果第一个值最大就 就输出0 第二个最大就输出1 ... 依次类推；
# 可以转化为第n个数据最大的概率，依次类推
def build_sample():
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        print("输入：%s, 第 %s 个参数最大, 概率值：%s" % (vec, torch.argmax(res).item(), res))   # 打印结果
