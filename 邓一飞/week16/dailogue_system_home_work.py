...
class DialogSystem:
    ...
    def load(self):
        ...
        # 加载重答模板
        self.load_reask_template("scenario/reask.json")
    ...
    def load_reask_template(self, reask_template_file):
        with open(reask_template_file,'r',encoding='utf-8') as f:
            self.reask_template = json.load(f)
    ...
    def reask(self, memory):
        user_input = memory["user_input"]
        max_score = -1
        for reask_template in self.reask_template:
            score = self.get_sentence_similarity(user_input,reask_template)
            if score > max_score:
                max_score = score
        if max_score > 0.3 and memory.get("bot_response") is not None:
            return True
        return False
