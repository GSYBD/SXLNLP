...
class DataGenerator:
    ...
    def load(self):
        ...
        if "json" in self.path:
            ...
            data = pd.read_csv(self.path, encoding='utf-8').to_numpy()
            label_class = []
            for label, str in data:
                if self.config["model_type"] == "bert":
                    input_id = self.tokenizer.encode(str, max_length=self.config["max_length"],
                                                     pad_to_max_length=True)
                else:
                    input_id = self.encode_sentence(str)
            ...
            self.config["class_num"] = len(label_class)
        else:
            pass
        return
