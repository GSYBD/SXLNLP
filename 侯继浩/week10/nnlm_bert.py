...
class LanguageModel(nn.Module):
    ...
    def forward(self, x, y=None):
        if y is not None:
            mask_np = np.ones((x.shape[0], x.shape[1], x.shape[1]))
            for i in range(mask_np.shape[0]):
                for j in range(mask_np.shape[1]):
                    for k in range(mask_np.shape[2]):
                        if k > j:
                            mask_np[i][j][k] = 0
            mask = torch.tensor(mask_np, dtype=torch.int8)
...
def build_sample(window_size, corpus):
    ...
    # print(window, target)
    x = tokenizers.encode(window, add_special_tokens=False, truncation=True, padding='max_length', max_length=16)   #将字转换成序号
    y = tokenizers.encode(target, add_special_tokens=False, truncation=True, padding='max_length', max_length=16)
    return x, y
...
def generate_sentence(openings, model, window_size):
    ...
    with torch.no_grad():
        ...
        while pred_char != "\n" and len(openings) <= 30:
            openings += pred_char
            x = [tokenizers.encode(char, add_special_tokens=False) for char in openings[-window_size:]]
            x = torch.LongTensor([x])
            if torch.cuda.is_available():
                x = x.cuda()
            y = model(x)[0][-1]
            index = sampling_strategy(y)
            pred_char = tokenizers.decode(index)
    return openings
