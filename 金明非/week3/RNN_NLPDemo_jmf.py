...
class TorchModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        output, x = self.rnn(x)                    # (batch_size, vector_dim, sen_len)->(batch_size, vector_dim)
        x = x.squeeze()
...
def build_sample(vocab, sentence_length):
    ...
    #指定哪些字出现时显示该索引列表
    # if set("你a") & set(x):
    #     y = [1 if c in "你a" else 0 for c in x]
    # #指定字都未出现，则为0
    # else:
    #     y = [0] * sentence_length
    if '你' in x:
        y = [1 if c in "你" else 0 for c in x]
    else:
...
if __name__ == "__main__":
    # main()
    test_strings = ["f你cfee", "cbdafb", "你ccde你", "eafa你f"]
