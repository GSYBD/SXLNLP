...
class DataGenerator:
        ...
        with open(self.path, encoding="utf8") as f:
            for line in f:
                ...
                if isinstance(line, dict):
                    ...
                    # CHANGE: 这里把label也加进来
                    label2id = self.encode_sentence(label)
                    label2id = torch.LongTensor(label2id)
                    self.knwb[self.schema[label]].append(label2id)
                #加载测试集
                else:
                    self.data_type = "test"
    ...
    #正样本从随机一个标准问题中随机选取两个
    #CHANGE: 使用triplet loss, 随机选取3个句子, s1和s2是同标签的, s3是不同标签的
    def random_train_sample(self):
        ...
        # s1和s2是同标签的, s3是不同标签的
        standard_question_index = list(self.knwb.keys())
        p, n = random.sample(standard_question_index, 2) # p是同组,n是不同组
        s1, s2 = random.sample(self.knwb[p], 2)
