...
def main():
    model = load_word2vec_model(r"D:\BaiduNetdiskDownload\week5 词向量及文本向量 2\week5 词向量及文本向量\model.w2v")  # 加载词向量模型
    sentences = load_sentence(r"D:\BaiduNetdiskDownload\week5 词向量及文本向量 2\week5 词向量及文本向量\titles.txt")  # 加载所有标题
    ...
    for label, distance in list(cluster_avg_distance_dict.items())[:15]:
        print("cluster %s :" % label)
        print("average distance: ", distance)
        for i in range(min(25, len(sentence_label_dict[label]))):
