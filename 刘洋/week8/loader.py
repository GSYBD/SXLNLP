...
class DataGenerator:
    ...
    def load(self):
        ...
        with open(self.path, encoding="utf8") as f:
            for line in f:
                ...
                #加载训练集
                # isinstance()函数用于检查一个对象是否属于某个特定的类或类型,这里用来检查line是否是字典类型,结果返回True或False
    ...
    def __getitem__(self, index):
        if self.data_type == "train":
            return self.random_Triplet_Loss() #随机生成一个训练样本,使用random_Triplet_Loss方法上送三个样本
        else:
    ...
    #随机生成2个相似样本和1个不相似样本
    def random_Triplet_Loss(self):
        standard_question_index = list(self.knwb.keys())
        # print(standard_question_index)
        p, n = random.sample(standard_question_index, 2)
        if len(self.knwb[p]) < 2:
            return self.random_Triplet_Loss()
        else:
            s1, s2 = random.sample(self.knwb[p], 2)
            print(s1, s2)
...
if __name__ == "__main__":
    ...
    # path=os.chdir("D:\\刘洋\\刘洋(个人)\\2024大数据算法学习\\学习资料\\第八周 文本匹配问题\\week8 文本匹配问题\\data")
    # print(path)
    print(enumerate(dg))
    train_loss=[]
