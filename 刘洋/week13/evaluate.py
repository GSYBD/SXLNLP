...
class Evaluator:
    ...
    def eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            ...
            print('我是labels{}'.format( len(labels)))
            with torch.no_grad():
                pred_res = self.model(input_id) #不输入labels，使用模型当前参数进行预测
                print('我是input_id{}'.format( len(pred_res)))
            self.write_stats(labels, pred_res, sentences)
            self.show_stats()
            return

    def write_stats(self, labels, pred_res, sentences):
        assert len(labels)== len(pred_res) == len(sentences)
        if not self.config["use_crf"]:
            pred_res = torch.argmax(pred_res, dim=-1)
        for true_label, pred_label, sentence in zip(labels, pred_res, sentences):
            if not self.config["use_crf"]:
...
if __name__ == "__main__":
    ...
    test=Evaluator(Config,model, logger)
    print(test.eval(1))
