...
规律：x是一个5维向量，取出5个数中的最小值，若为第一个位置则输出[1,0,0,0,0]
...
def build_sample():
    ...
    y = [0,0,0,0,0]
    y[find_min_index(x)] = 1
    return x, y

def find_min_index(x):
    min_index = min(enumerate(x), key=lambda pair: pair[1])[0]
    return min_index

def find_max_index(x):
    max_index = max(enumerate(x), key=lambda pair: pair[1])[0]
    return max_index
...
def evaluate(model):
    ...
    with torch.no_grad():
        ...
        for y_p, y_t in zip(y_pred, y):  # 与真实标签进行对比
            if find_max_index(y_p) == find_max_index(y_t):
                correct += 1  # 样本判断正确
            else:
                wrong += 1
...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        print(f"输入：{vec},实际结果：{find_min_index(vec)}, 预测结果：{find_max_index(res)}, 概率值：{res}")  # 打印结果
