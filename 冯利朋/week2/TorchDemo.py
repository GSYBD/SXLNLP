...
class TorchModel(nn.Module):
    def __init__(self, input_size):
        ...
        self.linear = nn.Linear(input_size, 1) # 全连接层
        self.activate = nn.Sigmoid() # 激活函数
...
def evaluate(model:TorchModel):
    ...
    print("本次一共生成%d个样本,其中正样本%d个,负样本%d个" % (eval_sample_total,sum(y), eval_sample_total - sum(y)))
    model.eval()
    with torch.no_grad():
        y_pred = model(x)
        right, wrong = 0, 0
...
def predict(model_path,predict_data,input_size):
    ...
    for input_data, y_p in zip(predict_data, y_pred):
        print("本次预测:%s,概率为%f,结果为%f" % (input_data,float(y_p), round(float(y_p))))
