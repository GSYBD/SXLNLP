...
"""
1. Embedding层参数
Embedding层主要由三部分组成：token embedding、position embedding和segment embedding。
...
size = 30522* 768 +512 * 768+512 * 768 +2 * 768 = 23835648

2. Multi-Head Attention层参数
12个头，
每个头的Q、K、V权重矩阵的维度为768 * (768 / 12)（即768 * 64），输出权重矩阵的维度为768 * 768。
单个Multi-Head Attention层的参数为(768 * 64 * 3) * 12 + 768 * 768。
size = 12 * [(768 * 64 * 3) * 12 + 768 * 768] = 28,311,552。

3. Layer Normalization层参数
它包含两个参数：gamma和beta。
每个Layer Normalization层有2 * 768个参数。
因为有三个地方使用到Layer Normalization（embedding层后、Multi-Head Attention层后、feed forward层后），
size=(2 * 768) * (1 + 12 + 12)= 38,400。

4. Feed Forward层参数
Feed Forward层主要由两个全连接层组成，每个全连接层都有自己的权重矩阵。
因此，单个Feed Forward层的参数为2 * (768 * 3072)。
size = 12 * 2 * (768 * 3072) = 56,623,104。

5.总数
23835648+28,311,552+38,400+56,623,104=108808704
"""

bert = BertModel.from_pretrained(r"C:\Users\86358\Desktop\nlp\课件\week6 语言模型\bert-base-chinese", return_dict=False)
