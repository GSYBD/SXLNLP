...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        if model_type == "fast_text":
            ...
            self.use_bert = True
            self.encoder = BertModel.from_pretrained(config["pretrain_model_path"], return_dict=False) #我首先要把bert import进来，一定要读取它预训练的权重
            #也可以试一下只用bert模型但不加载它训练好的权重，再对比加载预训练权重的效果。这里可以很明确的看出预训练是有意义的
    ...
    def forward(self, x, target=None):
        if self.use_bert:  # bert返回的结果是 (sequence_output, pooler_output)
            #sequence_output:batch_size, max_len, hidden_size --每个字对应的向量
            #pooler_output:batch_size, hidden_size  -- 整句话的向量
            x = self.encoder(x) #bert作为encoder来使用，使用bert 与使用lSTM、RNN、CNN最大的区别在于，它不需要embedding了，它自身就带embedding
        else:
        ...
        if isinstance(x, tuple):  #RNN类的模型会同时返回隐单元向量，我们只取序列结果
            x = x[0]
            # print('sequence_output形状',x.shape)
            #bert就取的是sequence_output，就是每个字对应的向量
        #可以采用pooling的方式得到句向量
        if self.pooling_style == "max": #可以根据自己的参数决定是做max-pooling还是average-pooling，这个参数设计在config文件中
            self.pooling_layer = nn.MaxPool1d(x.shape[1]) #有了每个字对应的向量我们就可以做pooling
        else:
            self.pooling_layer = nn.AvgPool1d(x.shape[1])
        x = self.pooling_layer(x.transpose(1, 2)).squeeze() #input shape:(batch_size, sen_len, input_dim)

        #也可以直接使用序列最后一个位置的向量
        # x = x[:, -1, :] #有的时候用LSTM的时候也可以取最后一个位置的向量
        predict = self.classify(x)   #input shape:(batch_size, input_dim) #分类层就是线性层，从768映射到类别个数
        if target is not None:
            return self.loss(predict, target.squeeze())
        # 交叉熵要求：y_pred(batch_size，class_num), y(batch_size)
        else:
            return predict
...
class BertLSTM(nn.Module):
    ...
    def forward(self, x):
        x = self.bert(x)[0] #把bert的第一个输出取出来
        x, _ = self.rnn(x) #送入lstm
        return x
