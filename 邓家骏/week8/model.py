...
class SiameseNetwork(nn.Module):
    ...
    # triplet loss
    def forward(self,sa, sp=None, sn=None, target=None):
                #同时传入三个句子
        if sp is not None:
            vector1 = self.sentence_encoder(sa) #vec:(batch_size, hidden_size)
            vector2 = self.sentence_encoder(sp)
            vector3 = self.sentence_encoder(sn)
