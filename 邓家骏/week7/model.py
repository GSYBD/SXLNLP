...
class TorchModel(nn.Module):
    def __init__(self, config):
        ...
        if model_type == "fast_text":
            ...
            hidden_size = self.encoder.bert.config.hidden_size
            # 不更新bert参数，额外加可训练一层FFN
        elif model_type == 'bert_diy':
            self.use_bert = True
            model = BertFFN(config)
            for name,param in model.named_parameters():
                if 'intermediate' not in name and 'outLinear' not in name :
                    param.requires_grad = False
            # 只训练最后一层参数
            # unfreeze_layers = ['layer.1','pooler']
            # for name,param in model.named_parameters():
            #     for el in unfreeze_layers:
            #         if el in name:
...
class BertFFN(nn.Module):
    def __init__(self, config):
        ...
        self.intermediate = nn.Linear(self.bert.config.hidden_size,int(4*self.bert.config.hidden_size))
        self.gelu = torch.nn.functional.gelu
        self.outLinear = nn.Linear(int(4*self.bert.config.hidden_size),self.bert.config.hidden_size)
    
       # ff_intermediate_w = h * 4h 
    def layer_FFN(self,x):
