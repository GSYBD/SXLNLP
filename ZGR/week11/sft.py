...
def build_vocab(vocab_path):
    vocab = {"<pad>": 0, "<UNK>": 1}  # 添加一个UNK token
    with open(vocab_path, encoding="utf8") as f:
        for index, line in enumerate(f):
            char = line.strip()  # 去掉结尾换行符
...
def build_sample(vocab, window_size, corpus):
    ...
    # 确保索引在词汇表范围内
    x = [i if i < len(vocab) else vocab["<UNK>"] for i in x]
    y = [i if i < len(vocab) else vocab["<UNK>"] for i in y]
...
def train(corpus_path, vocab_path, save_weight=True):
    epoch_num = 20  # 训练轮数
    ...
    for epoch in range(epoch_num):
        ...
        logging.info(generate_sentence("让他在半年之前，就不能做出", model, vocab, window_size))
        logging.info(generate_sentence("李慕站在山路上，深深的呼吸", model, vocab, window_size))
...
if __name__ == "__main__":
    train("corpus.txt", "vocab.txt", save_weight=False)
