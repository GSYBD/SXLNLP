...
class TorchModel(nn.Module):
    def __init__(self, vector_dim, vocab, sentence_length):
        ...
        self.linear = nn.Linear(vector_dim, sentence_length + 1)  # 线性层
...
def build_sample(vocab, sentence_length):
    ...
    x = [random.choice(a) for _ in range(sentence_length - 1)]
    x.append('a')
    random.shuffle(x)
    #指定哪些字出现时为正样本
    y = x.index('a') + 1
...
def predict(model_path, vocab_path, input_strings):
    ...
    for i, input_string in enumerate(input_strings):
        print(input_string, result[i])

if __name__ == "__main__":
    main()
    input_strings = ['hjkloiau','bnmhaudy']
