...
def build_sample(tokenizer, window_size, corpus):
    ...
    window = corpus[window_size]['title']
    target = corpus[window_size]['content']  # 输入输出错开一位
...
# window_size 样本长度
# corpus 语料字符串
def build_dataset(max_length, tokenizer, batch_size, corpus):
    dataset = []
    for i, (prompt, answer) in enumerate(corpus):
        ...
        # 构建一个mask矩阵，让prompt内可以交互，answer中上下文之间没有交互
        mask = generate_special_mask(len(prompt_encode), len(answer_encode))
        # padding
        x = x[:max_length] + [0] * (max_length - len(x))
        y = y[:max_length] + [0] * (max_length - len(y))
        x = torch.LongTensor(x)
        y = torch.LongTensor(y)
        mask = generate_pad_mask(mask, (max_length, max_length))
...
def generate_special_mask(s1, s2):
    len_s1 = s1 + 2  # cls+sep
    len_s2 = s2 + 1  # sep
    # 创建掩码张量
    ...
    for i in range(len_s2):
        # s2的当前token不能看到后面的s2token
        mask[len_s1 + i, len_s1 + i + 1:] = 0
    return mask


def generate_pad_mask(tensor, target_shape):
    # 获取输入张量和目标形状的长度
...
def train(corpus_path, save_weight=True):
    ...
    # train_sample = 1  # 每轮训练总共训练的样本总数
    char_dim = 768  # 每个字的维度
    # window_size = 1000  # 样本文本长度
    max_lenth = 100
    vocab_size = 21128  # 字表大小
    learning_rate = 0.001  # 学习率

    pretrain_model_path = r"F:\学习资料\PYTHON\课程\第六周 预训练模型\bert-base-chinese"
    tokenizer = BertTokenizer.from_pretrained(pretrain_model_path)

    corpus = load_corpus(corpus_path)  # 加载语料
    train_data = build_dataset(max_lenth, tokenizer, batch_size, corpus)
    ...
    for epoch in range(epoch_num):
        ...
        print(generate_sentence("震惊，南京长江大桥发生事故", model, tokenizer))
        print(generate_sentence("李嘉诚长江实业今天宣布破产", model, tokenizer))
    if not save_weight:
        return
    else:
...
if __name__ == "__main__":
    ...
    # 示例：生成一个5x5的矩阵
    # n = 5
    # matrix = generate_special_matrix(n)
    #  print(matrix)
    # height, width = 5, 6
    # x = torch.empty((height, width))  # 只是一个示例，x的内容不重要
    # mask = generate_special_matrix(x)
    # print(mask)
