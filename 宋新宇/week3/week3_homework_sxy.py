...
class TorchRNN(nn.Module):
    def __init__(self, vector_dim, sentence_length, hidden_side, vocab):
        ...
        self.layer = nn.Linear(hidden_side, sentence_length)  #线性层
...
def build_vocab():
    chars = "abcdefghijklmnopqrstuvwxyz你我他她好人工智能NLP"  #字符集
    vocab = {"pad":0}
    for index, char in enumerate(chars):
        vocab[char] = index + 1   #每个字对应一个序号
    vocab['unk'] = len(vocab) #38
    return vocab
...
def predict(model_path, vocab_path, input_strings):
    ...
    for input_string, res in zip(input_strings, y_p):
        print("输入：%s, 预测类别：%d, 概率值：%f" % (input_string, np.argmax(res.numpy()) + 1, np.max(res.numpy()))) #打印结果
...
if __name__ == "__main__":
    # main()
    print("\n固定字符串测试：")
    test_strings1 = ["fn好fee", "wzsd好g", "r好wdeg", "好akwww"]
    predict("model.pth", "vocab.json", test_strings1)

    print("====================================================")
    print("\n随机字符串测试：")
    test_strings2 = []
    vocab = build_vocab()
    vocab_copy1 = copy.deepcopy(list(vocab.keys()))
    vocab_copy1.remove('好')
    for i in range(10):
        x = [random.choice(vocab_copy1) for _ in range(5)]
        x.append('好')
        random.shuffle(x)
        test_strings2.append(x)
    predict("model.pth", "vocab.json", test_strings2)