...
class DataGenerator:
    ...
    def random_train_sample(self):
        # 随机正样本
        positive_index = list(self.knwb.keys())
        p = random.choice(positive_index)
        #如果选取到的标准问下不足两个问题，则无法选取，所以重新随机一次
        if len(self.knwb[p]) < 2:
            return self.random_train_sample()
        else:
            # 随机负样本
            negative_index = [item for item in positive_index if item != p]
            n = random.choice(negative_index)
            #返回
