...
'''
#加载预训练模型
bert = BertModel.from_pretrained(r"D:\培训课程\NLP\第六周 预训练模型\bert-base-chinese\bert-base-chinese", return_dict=False)
...
class DiyBert:
    ...
    #bert embedding，使用3层叠加，再经过一个Layer norm层，embedding层实现
    ...
    def self_attention(self,
        ...
        q = np.dot(x, q_w.T) + q_b  # shape: [max_len, hidden_size]      W * X + B lINER    #可训练参数q_w，大小为4*768，可训练参数q_b，大小为4*768
        k = np.dot(x, k_w.T) + k_b  # shpae: [max_len, hidden_size]                         #可训练参数k_w，大小为4*768，可训练参数k_b，大小为4*768
        v = np.dot(x, v_w.T) + v_b  # shpae: [max_len, hidden_size]                         #可训练参数k_w，大小为4*768，可训练参数k_b，大小为4*768
        ...
        #结果作为全连接层的输入
        attention = np.dot(qkv, attention_output_weight.T) + attention_output_bias          #可训练参数attention_output_weight，大小为4*768，可训练参数attention_output_bias，大小为4*768
        return attention
    ...
    def feed_forward(self,
        ...
        x = np.dot(x, intermediate_weight.T) + intermediate_bias                            #可训练参数intermediate_weight，大小为3072*768，可训练参数attention_output_bias，大小为1*3072
        x = gelu(x)
        # output shpae: [max_len, hidden_size]
        x = np.dot(x, output_weight.T) + output_bias                                        #可训练参数output_weight，大小为768*3072，可训练参数output_bias，大小为1*768
        return x
    ...
    #链接[cls] token的输出层，池化层
    def pooler_output_layer(self, x):
        x = np.dot(x, self.pooler_dense_weight.T) + self.pooler_dense_bias                  #可训练参数pooler_dense_weight，大小为768*1，可训练参数pooler_dense_biass，大小为768*1
        x = np.tanh(x)
        return x
...
'''
该模型的可训练参数有:
    self_attention中的可训练参数q_w，大小为4*768，可训练参数q_b，大小为4*768；可训练参数k_w，大小为4*768，可训练参数k_b，大小为4*768；可训练参数k_w，大小为4*768，可训练参数k_b，大小为4*768；可训练参数attention_output_weight，大小为4*768，可训练参数attention_output_bias，大小为4*768
    前馈网络中的可训练参数intermediate_weight，大小为3072*768，可训练参数attention_output_bias，大小为1*3072；可训练参数output_weight，大小为768*3072，可训练参数output_bias，大小为1*768
    token的输出池化层的可训练参数pooler_dense_weight，大小为768*1，可训练参数pooler_dense_biass，大小为768*1
'''