...
bert = BertModel.from_pretrained(r"/Users/liuran/Desktop/八斗精品班/第六周 预训练模型/week6 语言模型和预训练/下午/bert-base-chinese", return_dict=False)
...
embeddings_vocab = c.vocab_size * hidden_size
embeddings_position = c.max_position_embeddings * hidden_size
embeddings_segment = c.type_vocab_size * hidden_size
embeddings_ln = 2 * hidden_size
#----------Embedding层的 总参数量----------
embeddings_nums = embeddings_vocab + embeddings_position + embeddings_segment + embeddings_ln
# Transformer参数:
# Q k v 多头
attention_qkv = 3 * hidden_size * hidden_size / c.num_attention_heads
attention_output_layer = hidden_size * hidden_size
attention_parmenters = hidden_size * hidden_size
attetion_bias = c.num_attention_heads * c.max_position_embeddings * c.max_position_embeddings
attetion_ln = 2 * hidden_size
self_attention_nums = attention_qkv + attention_output_layer + attention_parmenters + attetion_bias + attetion_ln
#前馈网络参数:
feedforward_intermediate = hidden_size * c.intermediate_size
feedforward_output_layer = c.intermediate_size * hidden_size
feedforward_ln = 2 * hidden_size
feedforward_nums = feedforward_intermediate + feedforward_output_layer + feedforward_ln
# ----------Transformer总参数:----------
transformer_nums =  (self_attention_nums + feedforward_nums) * c.num_hidden_layers
# pooling层
