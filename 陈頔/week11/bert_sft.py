...
def build_dataset(tokenizer, corpus, max_length, batch_size):
    ...
    for i, (prompt, answer) in enumerate(corpus):
        ...
        # 构造标签序列y，其中提示文本部分用-1填充，回答文本部分保持原样，因为输入有cls和sep所以要多加个-1
...
def create_mask(s1, s2):
    ...
    for i in range(len_s1):
        ...
        # mask[i,j],i是行索引，j是列索引
        # 把s1对应的行中，s1后面也就是s2的列全部置为0
        mask[i, len_s1:] = 0
    # print(mask)
    # 遍历s2的每个token
    for i in range(len_s2):
        # s2的当前token不能看到后面的s2 token
        # 前面行索引的计算方式。由于len_s1，所以s2第一个元素位于第 len_s1 行，第二个元素位于第 len_s1 + 1 行，
        # 依此类推。len_s1 + i 表示 s2 中的后面的第 i+1 个元素
        # 只能看到本身及前面的元素，后面的置0
...
def pad_mask(tensor, target_shape):
    ...
    # 将原始张量对应的部分填充到全零张量中
    # 将tensor中h_start:h_end, w_start:w_end的部分复制到result中
