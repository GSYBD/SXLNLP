...
class Evaluator:
    ...
    def  eval(self, epoch):
        ...
        for index, batch_data in enumerate(self.valid_data):
            ...
            with torch.no_grad():
                pred_results = self.model(input_id) #不输入labels，使用模型当前参数进行预测 # lyu: 输出是每个字对应的 向量
    ...
    def write_stats(self, labels, pred_results, sentences):
        ...
        if not self.config["use_crf"]:  # lyu: 不考虑crf时，对应输出结果相当于只有发射矩阵，需要取最大值, 输出值是整数序列, 类似于[3,0,0]
        ...
        for true_label, pred_label, sentence in zip(labels, pred_results, sentences):
            ...
            # print("=+++++++++")
            # print(true_entities)  # lyu: defaultdict(<class 'list'>, {'LOCATION': ['北京'], 'PERSON': ['刘欢'], 'TIME': ['3月19日', '当年']})
            # print(pred_entities)  # lyu: defaultdict(<class 'list'>, {'TIME': ['3月19日']})
            # print('=+++++++++')
    ...
    def show_stats(self):  # lyu: 计算准确率、召回率、F1
        F1_scores = []
        for key in ["PERSON", "LOCATION", "TIME", "ORGANIZATION"]:
            # 正确率 = 识别出的正确实体数 / 识别出的实体数
            # 召回率 = 识别出的正确实体数 / 样本的实体数
    ...
    def decode(self, sentence, labels):
        ...
        for location in re.finditer("(04+)", labels): # lyu: 如果找到的是1个0 和若干个4，则返回的location.span()是(0, 5)
            s, e = location.span()
