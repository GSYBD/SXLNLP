...
规律：x是一个5维向量，若第1个数最大，则类别为1；若第二个数最大，则类别为2 …… 若第五个数最大，则类别为5
...
class TorchModel(nn.Module):
    def __init__(self, input_size, hidden_size1, hidden_size2):
        ...
        self.loss = nn.functional.cross_entropy # loss采用交叉熵
    ...
    def forward(self, x, y = None):
        x = self.layer1(x)
        x = self.layer2(x)
        y_pred = torch.softmax(x, dim=1) # softmax激活函数
        if y is not None:
            return self.loss(y_pred, y) # 预测值和真实值计算损失
        else:
            return y_pred

# 随机生成一个5维向量，若第1个数最大，则类别为1；若第二个数最大，则类别为2 …… 若第五个数最大，则类别为5
