...
class SiameseNetwork(nn.Module):
    ...
    def forward(self, sentence1, sentence2=None, sentence3=None):
        ...
        if sentence2 is not None and sentence3 is not None:
            vector_a = self.sentence_encoder(sentence1)  # vec:(batch_size, hidden_size)
            vector_p = self.sentence_encoder(sentence2)
            vector_n = self.sentence_encoder(sentence3)
...
if __name__ == "__main__":
    ...
    # y = model.forward1(s1, s2, l)

    s3 = torch.LongTensor([[0, 0, 0, 4], [0, 2, 3, 4]])
