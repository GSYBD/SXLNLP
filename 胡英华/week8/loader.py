...
class DataGenerator:
    ...
    def random_train_sample(self):
        ...
        #随机正样本
        if random.random() <= self.config["positive_sample_rate"]:
            a, n = random.sample(standard_question_index, 2)  # 选择两个target
            s3 = random.choice(self.knwb[n])
            #如果选取到的标准问下不足两个问题，则无法选取，所以重新随机一次
            if len(self.knwb[a]) < 2:
                return self.random_train_sample()
            else:
                s1, s2 = random.sample(self.knwb[a], 2)
                return [s1, s2, s3]
        #随机负样本
        else:
...
if __name__ == "__main__":
    ...
    # print(len(dg)) # 与Config["epoch_data_size"]有关
    # for batch in dl:
    #     print(batch)
    # 从200个样本里，分成200 / 32 训练批次，每个批次有32个数据样本
    # print(dg[3])
    # print(dl)
