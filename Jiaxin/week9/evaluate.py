...
class Evaluator:
    ...
    def write_stats(self, labels, pred_results, sentences):
        ...
        for true_label, pred_label, sentence in zip(labels, pred_results, sentences):
            ...
            pred_entities = self.decode1(sentence, pred_label)
            print("=+++++++++")
            print(true_entities)
            print(pred_entities)
            print('=+++++++++')
    ...
    def decode(self, sentence, labels):
        ...
        for location in re.finditer("(04+)", labels):
            s, e = location.span()
            results["LOCATION"].append(sentence[s-2:e-2])
        for location in re.finditer("(15+)", labels):
            s, e = location.span()
            results["ORGANIZATION"].append(sentence[s-2:e-2])
        for location in re.finditer("(26+)", labels):
            s, e = location.span()
            results["PERSON"].append(sentence[s-2:e-2])
        for location in re.finditer("(37+)", labels):
            s, e = location.span()
            results["TIME"].append(sentence[s-2:e-2])
        return results


    def decode1(self, sentence, labels):
        labels = "".join([str(x) for x in labels[:len(sentence)]])
        results = defaultdict(list)
        for location in re.finditer("(04+)", labels):
            s, e = location.span()
